...
if (MAX_DISPLAY_SPECIAL_PRODUCTS > 0 ) {
  ...
  if ($num_products_count) {
    ...
    while (!$specials->EOF) {
      ...
      $products_price = zen_mb_get_products_display_price_from_company($specials->fields['products_id']);
	...
	$products_model=$specials->fields['products_model'];
	...
	//comment our special content <div class='ribbon special' title=''>Special</div> 2015 07 27
	$msg_product="";
	 //show our product model at line 74
	  ...
	  array('params' => 'class="specialsListBoxContents mix col-lg-4 col-md-6 col-sm-4 col-xs-12"',
									...
									<div class="product">
										...
										'<div class="product-thumbnail">' .(($specials->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : zen_image(DIR_WS_IMAGES . $specials->fields['products_image'], $specials->fields['products_name'], IMAGE_PRODUCT_LISTING_HEIGHT, IMAGE_PRODUCT_LISTING_WIDTH) . '<div class="caption bottom-left">' . $companyDiscount . '</div></div></a>
										<div class="product-name-desc">
											<div class="product_name">') .  
												'<a href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'cPath=' . $productsInCategory[$specials->fields['products_id']] . '&products_id=' . $specials->fields['products_id']) . '">' . $products_name . '</a>'.$products_model.'<div>'.$companyprice.'</div>'.

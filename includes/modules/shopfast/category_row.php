...
if ($num_categories > 0) {
  ...
  while (!$categories->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContentsFromCompany"',
                                           'text' => '<div class="row"><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '">' . zen_image(DIR_WS_IMAGES . $categories->fields['categories_image'], $categories->fields['categories_name']) . '<div>'.$categories->fields['categories_name'].'</div>' . '</a></div>' 

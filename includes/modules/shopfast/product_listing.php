    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        if (isset($_GET['manufacturers_id'])) {
			 ...
			 $product_desc = zen_get_products_description($listing->fields['products_id']);
        	 $product_desc = zen_clean_html($product_desc);

			 $product_desc = ltrim(substr($product_desc, 0, $product_desc_length) . '..');
       ...
       // $lc_price = '<div class="price-box">'.zen_get_products_display_price($listing->fields['products_id']) . '</div>';
		$products_price = zen_mb_get_products_display_price_from_company($listing->fields['products_id']);
        $lc_align = 'right';
        $lc_text =  $lc_price;
...
// qty box with add to cart button
		...
		if (UN_MODULE_WISHLISTS_ENABLED) { $wishlist_link= '<a class="wishlink" title="Add to Wishlist" href="' . zen_href_link('wishlist', 'products_id=' . $listing->fields['products_id'] . '&action=wishlist_add_product') . '">+ Add to Wishlist</a>';}else{ $wishlist_link='';}
        ...
        if ($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          if (isset($_GET['manufacturers_id'])) {
            ...
            $product_img =  zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $products_name, IMAGE_PRODUCT_LISTING_HEIGHT, IMAGE_PRODUCT_LISTING_WIDTH);
          } else {
 ...
 $get_discount_prod=mb_discount_product($listing->fields['products_id']);
		 ...
		 //$is_newprod=(mb_new_product($listing->fields['products_id'])==1)? ' <div class="ribbon new"></div>' : '' ;
		 //$is_specials=(	mb_special_product($listing->fields['products_id'])==1)? ' <div class="ribbon special"></div>' : '' ;
		 if(mb_featured_product($listing->fields['products_id'])==1){
			// cut ribbon hot out 2015 07 20 $ribbon='<div class="ribbon hot"></div>';
			$ribbon='';
		}
		else if(mb_special_product($listing->fields['products_id'])==1){
			// cut ribbon hot out 2015 07 20 $ribbon='<div class="ribbon special"></div></div>';
			$ribbon='';
		}
		else if(mb_new_product($listing->fields['products_id'])==1){
				// cut ribbon hot out 2015 07 20 $ribbon='<div class="ribbon new"></div>';
	 ...
	 $addtocartbtn = zen_get_buy_now_button($listing->fields['products_id'],'<a class="add-to-cart product_cart_image" data-toggle="tooltip" data-original-title="Add to Cart" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing->fields['products_id']) . '"></a>');
	
	 $listview_addtocartbtn = zen_get_buy_now_button($listing->fields['products_id'],'<a class="btn btn-primary btn-iconed" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing->fields['products_id']) . '"><i class="icon-shopcart"></i><span>Add to Cart</span></a>');
		 

           //define our normal price 2015 07 27 
		 ...
		 //define our lc_text company price 2015 07 27 
		 $ourCompanyDiscount='';
      ...
      $lc_text = '<div class="product" data-filter="listing" data-name="'.$product_name_only.'" data-discount="'.$get_discount_prod.'" data-price="101">';
	  
	   $lc_text.='<a class="product-link clearfix" href="'.$product_link_only.'">';
	  		 $lc_text.= $ribbon;
			  $lc_text.='<div class="product-thumbnail">';
			     $lc_text.=$product_img;
				 $lc_text.='<div class="caption top-right"></div>';
			  	$lc_text.='<div class="caption bottom-left">'.$ourCompanyDiscount.'</div>';
			  $lc_text.='</div>';
	   $lc_text.='</a>';
	   $lc_text.='<div class="product-info clearfix">';
	   		$lc_text.='<h4 class="title"><a href="'.$product_link_only.'">'.$product_name_only.'</a>'.$listing->fields['products_model'].'b</h4>'.$products_model;
			$lc_text.='<div>'.$show_normal_price.'</div>';
			$lc_text.='<div class="description">';
			//do not show product_price_box 2015 07 27 tim
				//$lc_text.='<div class="prices">'.$product_price_box.'</div>';
				$lc_text.='<div class="text"><p>'.$product_desc.'</p></div>';
				$lc_text.='<div class="add-to-cart">'. $listview_addtocartbtn.'</div>';
				$lc_text.='<div class="overlay">
                 	<div class="product-actions">'.
                    	$wishlist_link .'
                        <a class="addtocompare" title="Add to Compare" href="javascript: compareNew('.$listing->fields["products_id"].', \'add\')">+ Add to Compare</a>
                    </div>
              	</div>';
			$lc_text.='</div>';
		     $lc_text.= $addtocartbtn;
	   $lc_text.='</div>';
	 $lc_text.='</div>';
	$lc_text.='</div>';

...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  ...
  $specials_index_query = "select p.products_id, p.products_model, p.products_image, pd.products_name, p.master_categories_id
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $list_of_products = substr($list_of_products, 0, -2); // remove trailing comma
	 //show product model ,add p.products_model column 2015 07 17 
    $specials_index_query = "select distinct p.products_id, p.products_model, p.products_image, pd.products_name, p.master_categories_id
                             from (" . TABLE_PRODUCTS . " p
                             left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id
                             left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id )
                             where p.products_id = s.products_id
...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
    $products_price = zen_mb_get_products_display_price_from_company($specials_index->fields['products_id']);
	//common ribbon special <div class='ribbon special' title=''>Special</div> 2015 07 22
	$msg_product="";
	...
	$products_model=$specials_index->fields['products_model'];
	...
	'<div class="product">
			...
			<div class="product-thumbnail">'.zen_image(DIR_WS_IMAGES . $specials_index->fields['products_image'], $specials_index->fields['products_name'], IMAGE_PRODUCT_LISTING_HEIGHT, IMAGE_PRODUCT_LISTING_WIDTH).'<div class="caption bottom-left">' . $companyDiscount . '</div></div></a>
		'.$wishlist_link.'
		<div class="product-name-desc">
			<div class="product_name">') .  
				'<a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'cPath=' . $productsInCategory[$specials_index->fields['products_id']] . '&products_id=' . $specials_index->fields['products_id']) . '">' . $products_name . '</a>'.$products_model.'<div>'.$companyprice.'</div>'.
	...
	$mybutton="<span id='mycompanybutton'>".strtoupper(TABLE_HEADING_SPECIALS_INDEX)."</span>
	           <span id='mycompanybuttonShopMore'><a href='specials.html'>shop more<span id='mycompanybuttonArrow'></span></a></span>";	 
  ...
  if ($specials_index->RecordCount() > 0) {
    $title = "<div class='box_heading box_heading_specials section'>
			  	 <h4 class='section-title'>". sprintf($mybutton, strftime('%B')) ." </h4> 

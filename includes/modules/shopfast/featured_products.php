...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
	//show product model ,add p.products_model column 2015 07 20
  $featured_products_query = "select distinct p.products_id,  p.products_model, p.products_image, pd.products_name, p.master_categories_id
		...
		//show product model ,add p.products_model column 2015 07 20				   
		 $last_featured_products_query = "select max(p.products_date_added),p.products_id, p.products_model, p.products_image, pd.products_name, p.master_categories_id
                           from (" . TABLE_PRODUCTS . " p
                           left join " . TABLE_FEATURED . " f on p.products_id = f.products_id
                           left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id )
                           where p.products_id = f.products_id
...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
	...
	//cancel ribbon hot 2015 07 20 tim	$msg_product="<div class='ribbon hot' title=''>Featured</div>";
	$msg_product="";
	...
	$products_model='<div style="color:#888888;font-size: 11px;">SKU: '.$products_model.'</div>';
	...
	<div class="product">
		...
		<div class="product-name-desc">
			<div class="product_name">') .  
				'<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' . $products_name . '</a>'.$products_model.''.

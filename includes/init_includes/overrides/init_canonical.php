 ...
 * @copyright Portions Copyright 2012 - 2015 Andrew Ballanger
 ...
 */
  case (strstr($current_page, '_info') && isset($_GET['products_id'])):
    $canonicalLink = zen_href_link($current_page, ($includeCPath ? 'cPath=' . zen_get_generated_category_path_rev(zen_get_products_category_id($_GET['products_id'])) . '&' : '') . 'products_id=' . $_GET['products_id'], $request_type, false);
    break;
/**
 * for product listings:
 */
  case ($current_page == 'index' && isset($_GET['cPath'])):
    $canonicalLink = zen_href_link($current_page, zen_get_all_get_params($excludeParams), $request_type, false);
    break;
/**
 * for music products:
 */
 ...
 */
  case ($this_is_home_page):
    $canonicalLink = preg_replace('/(index.php)(\?)(main_page=)(index)$/', '', zen_href_link(FILENAME_DEFAULT, '', $request_type, false));
    break;
/**
...
/**
 * Optional code to attempt and hide the canonical when the page request
 * was for the canonical URI. This currently has only been tested under
 * Apache web servers.
 *
 * Uncomment the next three lines to enable.
 */
//$my_url = ($request_type == 'SSL' ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
//if($canonicalLink == $my_url) $canonicalLink = '';
//unset($my_url);

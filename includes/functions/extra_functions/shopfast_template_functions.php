...
{
	global $db;
    $tr_qry = "SELECT * FROM ".TABLE_SHOPFAST_TEMPLATE_CONFIG." WHERE mb_keys='".$keys."' and languages_id='".$lang_id."'";
...
{
	global $db;
	$chk_keys = "select * from ".TABLE_SHOPFAST_TEMPLATE_CONFIG." WHERE mb_keys='".$key."' and languages_id='".$lang_id."'";
	$res_chk_keys = $db->Execute($chk_keys);
	$numsrows_chek_keys = $res_chk_keys->RecordCount();
	if($numsrows_chek_keys==0)
	{
	     $tr_qry = "INSERT INTO ".TABLE_SHOPFAST_TEMPLATE_CONFIG."(languages_id,mb_keys,mb_values) VALUES('".$lang_id."','".$key."','".$value."')";
	}else
	{
		 $tr_qry = "UPDATE ".TABLE_SHOPFAST_TEMPLATE_CONFIG." set mb_values='".$value."'  WHERE mb_keys='".$key."' and languages_id='".$lang_id."'";
	}
	$tr_config = $db->Execute($tr_qry);
...
// show case only
      ...
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
          if ($display_normal_price != 0) {
            ...
            $show_discount_amount = '';
          }//change show like discount 0ff 2015 07 23 tim    '<span class="productPriceDiscount">' .'-'. $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span>';
          $show_sale_discount = '<span class="productPriceDiscount">'.  $show_discount_amount .'<span>'. PRODUCT_PRICE_DISCOUNT_PERCENTAGE . ' Off</span></span>';
        ...
        if (SHOW_SALE_DISCOUNT == 1) {
          $show_sale_discount = '<span class="productPriceDiscount">' . number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . '<span>'. PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span></span>';
        } else {
    ...
    if ($display_normal_price == 0) {
      // don't show the $0.00
      $final_display_price = $show_special_price .$show_sale_discount. $show_sale_price ;
    } else {
      $final_display_price = $show_normal_price.$show_sale_discount. $show_special_price . $show_sale_price;
    }
...
//create new method for show  product price 2015 07 27 
function zen_mb_get_products_display_price_from_company($products_id,$sales='0') {
    global $db, $currencies;
...
// show case only
    ...
    if ($display_special_price) {
      $show_normal_price = '<span class="normalpriceCompany">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>'; //id="old-price-'.$products_id.'"
	  
      if ($display_sale_price && $display_sale_price != $display_special_price) {
      
		 $show_special_price = '<span class="productSpecialPriceSaleCompany">' .$currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>'; //id="product-price-'.$products_id.'"
		
        if ($product_check->fields['product_is_free'] == '1') {
		   $show_sale_price = '<span class="productSalePriceCompany">' .$currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])). '</span>'; //id="product-price-'.$products_id.'"
        ...
        if ($product_check->fields['product_is_free'] == '1') {
			$show_special_price = '<span class="productSpecialPriceCompany">'  .$currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>'; //id="product-price-'.$products_id.'"
        } else {
		    $show_special_price = '<span class="productSpecialPriceCompany">' .$currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>'; //id="product-price-'.$products_id.'"
      ...
      if ($display_sale_price) {
        ...
        $show_normal_price = ' <span class="normalpriceCompany">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>'; //id="old-price-'.$products_id.'"
        $show_special_price = '';
		  $show_sale_price = '<span class="special-price productSalePriceCompany"><span  class="price product-price-'.$products_id.'">' .$currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span></span>'; //id="product-price-'.$products_id.'"
    
      } else {
        if ($product_check->fields['product_is_free'] == '1') {
          $show_normal_price = '<span class="normalpriceCompany">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>'; //id="old-price-'.$products_id.'"
        } else {
          $show_normal_price = '<span class="single_priceCompany">'. $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])).'</span>'; //id="product-price-'.$products_id.'"
    ...
    if ($display_normal_price == 0) {
      // don't show the $0.00
	  //return array for final_display_price  the order is  $show_special_price $show_sale_discount $show_sale_price show_normal_price  2015 07 27 tim
  $final_display_price =array('show_special_price'=>$show_special_price,'show_sale_discount'=>$show_sale_discount,'show_sale_price'=>$show_sale_price,'show_normal_price'=>$show_normal_price,'show_all_price'=>$show_special_price.$show_sale_discount.$show_sale_price.$show_normal_price);
    } else {
		 //return array for final_display_price  the order is  $show_normal_price $show_sale_discount $show_special_price  $show_sale_price   $show_normal_price 2015 07 27 tim
	 ...
	 $cont.= '<div class="display-mode"><ul class="unstyled float-right">
			  ';
			     (defined('PRODUCT_LISTING_LAYOUT_STYLE')? PRODUCT_LISTING_LAYOUT_STYLE: 'rows');
				   ...
				   {   
						$cont.= '<a class="grid" title="'.HEADING_VIEW_AS_GRID.'" href="'.zen_href_link($page, zen_get_all_get_params(array('view')) . 'view=columns').'" >'.'<i class="fa fa-th fa-lg"></i>'.HEADING_VIEW_AS_GRID.'</a>&nbsp;';
						$cont.= '<div class="list" title="'.HEADING_VIEW_AS_LIST.'">'.'<i class="fa fa-th-list fa-lg"></i>'.HEADING_VIEW_AS_LIST.'</div>';
				   }
				   else
				   {
						$cont.= '<div class="grid" title="'.HEADING_VIEW_AS_GRID.'">'.'<i class="fa fa-th fa-lg"></i>'.HEADING_VIEW_AS_GRID.'</div>';
						$cont.= '<a class="list" title="'.HEADING_VIEW_AS_LIST.'" href="'.zen_href_link($page, zen_get_all_get_params(array('view')) . 'view=rows').'" >'.'<i class="fa fa-th-list fa-lg"></i>'.HEADING_VIEW_AS_LIST.'</a>&nbsp;';
				   }
		$cont.= '</ul></div>';  
		return $cont;
 }

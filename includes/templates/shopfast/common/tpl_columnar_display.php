...
if (is_array($list_box_contents) > 0 ) 
	{
		...
		if($param1 == "categoryListBoxContentsFromCompany") 
		{ 
		?>
        			...
        			for($col=0;$col<sizeof($list_box_contents[$row]);$col++) 
						{
							...
							{   
								echo '<li  class="categoryListBoxContentsFromCompany col-md-6 col-sm-6 col-lg-6 col-xs-12">' . $list_box_contents[$row][$col]['text'] .  '</li>' . "\n";
					 		}
    					}
  				} ?>
              	</ul></div>
		...
		elseif($param4 == "itemcenterBoxContentsFeatured" && !$this_is_home_page)
		{ ?>
        	<div class="row">	


              <!--   I make change for show cartgories 2015 07 24
                <div id="featured-slider-inner" class="owl-carousel owl-theme products-container">
                <?php
                for($row=0;$row<sizeof($list_box_contents);$row++)
                    {
                        $params = "";
                        ...
                        for($col=0;$col<sizeof($list_box_contents[$row]);$col++) 
                            {
                                ...
                                if (isset($list_box_contents[$row][$col]['text'])) 
                                { 
                                    echo '<div class="mix col-xs-12 col-sm-6 col-lg-4 mix_all" style="display: gblock;  opacity: 1;">' . $list_box_contents[$row][$col]['text'] .  '</div>' . "\n";
                                }
                            }
                    }
                ?>
		...
		{ ?>
           	<div class="row">	
                <div id="new-slider" class="owl-carousel owl-theme products-container">
                <?php
                for($row=0;$row<sizeof($list_box_contents);$row++)
                    {
                        $params = "";
		...
		{ ?>
			<div class="row">		
                <div id="specials-slider" class="owl-carousel owl-theme products-container">
				<?php
                for($row=0;$row<sizeof($list_box_contents);$row++)
                    {
                        $params = "";
